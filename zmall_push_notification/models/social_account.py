...
class SocialAccountthenewpushNotifications(models.Model):
    ...
    # firebase_thenewpush_certificate_key = fields.Char('Firebase thenewpush Certificate Key', related='website_id.firebase_push_certificate_key')
    ...
    #notification_request_url = fields.Text('Notification Request URL' , default="something")
    ...
    def unlink(self):
        if not self.env.user.has_group('base.group_system') and any(account.website_id for account in self):
            raise UserError(_("You can't delete a the_new_push Notification account."))
        return super(SocialAccountthenewpushNotifications, self).unlink()
    ...
    def _firebase_send_message_from_configuration(self, data, visitors):
        ...
        results = []
        tokens = visitors.mapped('thenewpush_token')
        if firebase_admin and self._check_firebase_version():
            ...
            raise UserError(_('You have to either install "firebase_admin>=2.17.0" or '
                              '"google_auth>=1.18.0" to be able to send thenewpush '
                              'notifications.'))
    ...
    def _firebase_send_message_from_iap(self, data, visitors):
        ...
        #     jsonrpc(url_join(social_iap_endpoint, '/iap/social_thenewpush_notifications/firebase_send_message'), params=batch_data)
