# -*- coding: utf-8 -*-
# Part of Odoo. See LICENSE file for full copyright and licensing details.

import ast
import pytz

from odoo import models, fields, _
from odoo.osv import expression
import africastalking
import logging
import requests
_logger = logging.getLogger(__name__)

class SocialLivePostthenewpushNotifications(models.Model):
    _inherit = 'social.live.post'

    reached_visitor_ids = fields.Many2many('website.visitor', string="Reached Visitors")
    # username = fields.Char(default="ETTADEV")hours
    # api_key = fields.Char(default="101f5a1f7ec3aa0e5aebff3a95ba23a3a20aff9f00b18d9cbe8b6af5c540ba00")
    def _post(self):
                """ The _post method of thenewpush notifications, unlike other social.media, doesn't post messages directly
                Instead, we keep them 'ready' and they are gathered by a cron job (see 'social.post#_cron_publish_scheduled'). """
                _logger.info("###########################################################################################")
                # message =  "not found"
                # for line_post in self:
                # message = line_post.post_id.message
                # _logger.info(message)
                thenewpush_notifications_live_posts = self.filtered(lambda post: post.account_id.media_type == 'thenewpush_notifications')
                super(SocialLivePostthenewpushNotifications, (self - thenewpush_notifications_live_posts))._post()
                
                thenewpush_notifications_live_posts.write({
                    'state': 'ready'
                })
                _logger.info("***********Done*******************")
                # push_notifications_live_posts = self.filtered(lambda post: post.account_id.media_type == 'push_notifications')
                # super(SocialLivePostpushNotifications, (self - push_notifications_live_posts))._post()
                
                # push_notifications_live_posts.write({
                #     'state': 'ready'
                # })
                
                # url = "https://fcm.googleapis.com/fcm/send"
                # body =  {
                # "to" : "cFdaJ_8T900:APA91bEsbNj9cHXdCXx15WYjNQI2os8csfM1qLW7kQev4JGkcEG7cGjf3kt2gPxZdKP7qkqWpqSUnfaqoJexhCrDro4Y2ZMz01pb0zsSfrqACDbFfGaDcLc9rgs_DeWxHcD5bpPtVnan",
                # "collapse_key" : "type_a",
                # "notification" : {
                # "body" : message,
                # "title": "Zmall Delivery",
                # "sound": "Submarine.aiff"    
                #             }
                #         }
                # headers= {
                #         'Content-Type': 'application/json',
                #         'Content-Length': '5',
                #         'Authorization': "key=AAAAVIHhWAo:APA91bGo_7KRBU7WbFSYYopuWWbBInWcv0gakYQ2Z1MLohGdbXJ8TmlQBAwPVkkRb2DDmACCNeiNI2xDm463k9TIBKoTflkk-_SA8olbRKYNeWPKuEsxUSqtzUDz2xvmzuVfOkPRO17a"
                #         }          
                # r = requests.post(url, json = body, headers= headers)
                # _logger.info(r.status_code)
                # live_post.write({'state': 'posted'})
        # username = "ETTADEV"   # use 'sandbox' for development in the test environment
        # api_key = "101f5a1f7ec3aa0e5aebff3a95ba23a3a20aff9f00b18d9cbe8b6af5c540ba00"      # use your sandbox app API key for development in the test environment
        # africastalking.initialize(username, api_key)


        # # Initialize a service e.g. SMS
        # sms = africastalking.SMS


        # # Use the service synchronously
        # response = sms.send("Hello Message!", ["+251964345364"],"8165")
        # _logger.info(response)
        #print(response)
        # sms_notifications_live_posts = self.filtered(lambda post: post.account_id.media_type == 'sms_notifications')
        # super(SocialLivePostsmsNotifications, (self - sms_notifications_live_posts))._post()

        # sms_notifications_live_posts.write({
        #     'state': 'ready'
        # })

    def _post_thenewpush_notifications(self):
        _logger.info("wow")
        posted_list = self.env['social.post'].search([("state" , "=" , "posted")])
        for live_post in posted_list:
                _logger.info("yesssssssss")
                message = live_post.message
                user_domain = ast.literal_eval(live_post.user_domain)
                users = self.env['res.partner'].search(user_domain)
                _logger.info("**********############################################################")
                for every_user in users:
                        _logger.info(every_user)
                        url = "https://fcm.googleapis.com/fcm/send"
                        body =  {
                        "to" : every_user.x_devicetoken,
                        "collapse_key" : "type_a",
                        "notification" : {
                        "body" : message,
                        "title": "Zmall Delivery",
                        "sound": "Submarine.aiff"    
                                    }
                                }
                        headers= {
                                'Content-Type': 'application/json',
                                'Content-Length': '5',
                                'Authorization':"key=AAAAVIHhWAo:APA91bGo_7KRBU7WbFSYYopuWWbBInWcv0gakYQ2Z1MLohGdbXJ8TmlQBAwPVkkRb2DDmACCNeiNI2xDm463k9TIBKoTflkk-_SA8olbRKYNeWPKuEsxUSqtzUDz2xvmzuVfOkPRO17a"
                                }    
                        _logger.info(body)      
                        r = requests.post(url, json = body, headers= headers)
                        _logger.info(r.status_code)
                return True
                # live_post.write({'state': 'posted'})
        _logger.info("noooooooooooooooo")
    
    def _post_thenewpush_notifications(self, data):
        _logger.info("wow")
        posted_list = self.env['social.post'].search([("state" , "=" , "posted")])
        for live_post in posted_list:
                _logger.info("yesssssssss")
                message = live_post.message
                user_domain = ast.literal_eval(live_post.user_domain)
                users = self.env['res.partner'].search(user_domain)
                _logger.info("**********############################################################")
                for every_user in users:
                        _logger.info(every_user)
                        url = "https://fcm.googleapis.com/fcm/send"
                        body =  {
                        "to" : every_user.x_devicetoken,
                        "collapse_key" : "type_a",
                        "notification" : {
                        "body" : message,
                        "title": "Zmall Delivery",
                        # "target_url": data['target_url']
                        "sound": "Submarine.aiff"    
                                    }
                                }
                        headers= {
                                'Content-Type': 'application/json',
                                'Content-Length': '5',
                                'Authorization':"key=AAAAVIHhWAo:APA91bGo_7KRBU7WbFSYYopuWWbBInWcv0gakYQ2Z1MLohGdbXJ8TmlQBAwPVkkRb2DDmACCNeiNI2xDm463k9TIBKoTflkk-_SA8olbRKYNeWPKuEsxUSqtzUDz2xvmzuVfOkPRO17a"
                                }    
                        _logger.info(body)      
                        r = requests.post(url, json = body, headers= headers)
                        _logger.info(r.status_code)
                return True
                # live_post.write({'state': 'posted'})
        _logger.info("noooooooooooooooo")
            # post = live_post.post_id
            # account = live_post.account_id
            # title = post.thenewpush_notification_title or _('New Message')
            # icon_url = '/web/image/social.post/%s/thenewpush_notification_image/64x64' % post.id if post.thenewpush_notification_image else '/mail/static/src/img/odoobot_transparent.png'

            # # TODO awa: force thenewpush_token domain here in case user manually removed it in form view?
            # visitor_domain = ast.literal_eval(live_post.post_id.visitor_domain)
            # target_link = ''
            # if post.sms_notification_target_url:
            #     link_tracker_values = live_post._get_utm_values()
            #     link_tracker_values['url'] = post.sms_notification_target_url
            #     link_tracker = self.env['link.tracker'].create(link_tracker_values)
            #     target_link = link_tracker.short_url

            # if not post.use_visitor_timezone:
            #     target_visitors = self.env['website.visitor'].search(visitor_domain)
            # else:
            #     # We need to filter the target_visitors based on their timezone
            #     post_date = post.scheduled_date or live_post.create_date
            #     post_user_datetime = pytz.utc.localize(post_date).astimezone(pytz.timezone(post.create_uid.tz)).replace(tzinfo=None)
            #     now_utc = pytz.utc.localize(fields.Datetime.now())

            #     def get_filtered_timezone_visitors(visitor):
            #         visitor_tz = pytz.timezone(visitor.timezone or 'UTC')
            #         visitor_local_datetime = now_utc.astimezone(visitor_tz).replace(tzinfo=None)
            #         return visitor_local_datetime > post_user_datetime

            #     pending_visitors = self.env['website.visitor'].search(expression.AND([visitor_domain, [('id', 'not in', live_post.reached_visitor_ids.ids)]]))
            #     target_visitors = pending_visitors.filtered(get_filtered_timezone_visitors)

            # account._firebase_send_message({
            #     'title': title,
            #     'body': self.env['mail.render.mixin'].sudo()._shorten_links_text(post.message, live_post._get_utm_values()),
            #     'icon': icon_url,
            #     'target_url': target_link
            # }, target_visitors)

            # # TODO awa:
            # # In theory, we should clean registrations from database for which we receive a 'registration-token-not-registered' error.
            # # However, while running tests, I've seen that this error appears a lot more than it should.
            # # When looking into it, I've seen this topic: https://github.com/firebase/firebase-admin-node/issues/533
            # # It mentions how this error is kind of like a 'default' and could mean many things (firebase having issues contacting a service, ...)
            # # As long as we can't be sure that the token is actually expired, we can't risk to delete it.
            # # 2 possible solutions:
            # # - wait for them to break the error down into more specific codes (see github thread)
            # # - implement a mechanism where we remove the registration after having received X errors consecutively
            # # In the mean time, please DON'T UNCOMMENT this
            # # self._clean_unregistered_tokens(result)

            # if post.use_visitor_timezone:
            #     values = {
            #         'reached_visitor_ids': [(4, target_visitor.id) for target_visitor in target_visitors]
            #     }
            #     # If all visitors have been processed
            #     if len(pending_visitors) == len(target_visitors):
            #         values['state'] = 'posted'

            #     live_post.write(values)
            # else:
            #     live_post.write({'state': 'posted'})

    # TODO AWA : check what is result object and use token directly instead of id
    # as token are now stored on website_visitor
    def _clean_unregistered_tokens(self, result):
        """ This will clean the tokens for which we receive a 'registration-token-not-registered' error
        from firebase.
        This method assumes the 'responses' from the batches are ordered the same way as matching registrations.
        (In all test cases so far, they were) """

        matched_registrations = result[0]
        batch_results = result[1]

        all_responses = []
        for batch_result in batch_results:
            all_responses.extend(batch_result.responses)

        i = 0
        registration_token_to_remove = []
        for response in all_responses:
            i += 1
            if not response.success and response.exception.code == 'registration-token-not-registered':
                registration_token_to_remove.append(matched_registrations[i]['token'])

        if registration_token_to_remove:
            self.env['website.visitor'].search([('thenewpush_token', 'in', registration_token_to_remove)]).sudo().write({
                'thenewpush_token': False
            })
